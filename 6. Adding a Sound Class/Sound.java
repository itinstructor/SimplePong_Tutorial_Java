...
/**
 * This enumeration encapsulates all the sound effects of a game, so as to
 * separate the sound playing code from the game code.
 * 1. Define all your sound effect names and the associated wave files.
 * 2. To play a specific sound, invoke Sound.SOUND_NAME.play().
 * 3. Invoke the static method Sound.init() to
 *    pre-load all the sound files, so that the play is not paused
 *    while loading the file for the first time.
 * 4. Use the static variable Sound.volume to mute the sound.
 */
public enum Sound {
   ...
   public void play() {
      if (volume != Volume.MUTE) {
         if (clip.isRunning()) // Is the audio clip still playing?
   ...
   // Static method to pre-load all the sound files into memory
   static void init() {
